...
smp_dir = '/Users/colemankane/Desktop/crrel_exports/smp_profiles'
...
for file in strat_files:#[-2:-1]:
    print(file)
    id = f'{file.split('_')[0]}_{file.split('_')[1]}'
    smp_filename = id + '_smp.csv'

    try:
        smp_mat = pd.read_csv(os.path.join(data_dir, smp_filename))
        good_profs = smp_mat['Comments'].isna()
        good_profs = smp_mat['Y-Coord'] == -30
        smp_mat = smp_mat[good_profs]
        smp_sn = smp_mat['SN'].astype(int).astype(str).str.zfill(2)
        ...
        strat = pd.read_csv(data_dir + '/' + file)

        bottom = strat.index[strat['bottom_cm'] == 0][0]
        ...
        for i in strat_filtered.index:
            ...
            for j in smp_profs:
                ...
                layer_rho = smp_prof.loc[smp_layer_ix]['P2015_density [kg/m^3]'] # all density values from layer
                if layer_rho.empty:
                    rho_to_avg.append(np.nan)
                else:
            ...
            strat.at[i, 'hs_cm'] = delta
            strat.at[i, 'swe_mm'] = layer_swe
            strat.at[i, 'density_kgm3'] = layer_density

        strat.to_csv(data_dir + '/' + file)
